...
## 2026-04-18 · 图 5.10 性能曲线改为按智能体编号分列布局

### 背景
作者要求将图 5.10 (TAD 三算法关键性能曲线, 6 子图) 改为 3 行 2 列布局, 按 "1
在左列, 2 在右列" 分组 - 即所有编号 1 的子图在左列, 所有编号 2 的子图在右列.

### 修改内容
原布局 (按行, 每行 3 子图, 共 2 行):
```
[相对角度 1] [相对角度 2] [d_D1T]
[d_D2T]      [d_A1T]      [d_A2T]
```

新布局 (按列, 左列 = 1 号智能体, 右列 = 2 号智能体):
```
[相对角度 1]   [相对角度 2]
[d_D1T]        [d_D2T]
[d_A1T]        [d_A2T]
...
### 实现方式
使用两个 `\begin{minipage}[t]{0.46\linewidth}` 分别构造左右两列, 每列内部用
`\subcaptionbox{...}\\[6pt]` 垂直堆叠 3 个子图. 这种 2 列 minipage 布局比单纯
的 \subcaptionbox + \hfill 更适合按列优先排版.

### 编译验证
- ✅ main.pdf 共 83 页
- ✅ 6 个子图正确按列排列, 左列 1 号、右列 2 号
- ✅ 子图自动编号 (a)~(f) 无重复
