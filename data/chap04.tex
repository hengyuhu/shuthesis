...
\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.45]{hunt_scene.pdf}
  \vspace{4cm}
...
图~\ref{fig:tpf_field} 展示了威胁势场在博弈空间中的等高线分布.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{tpf_field.pdf}
  \vspace{4cm}
  \bicaption{威胁势场等高线分布示意}{Contour illustration of the threat potential field}
  \label{fig:tpf_field}
\end{figure}
...
对全局状态进行特征提取. 训练阶段采用 CTDE 范式, 执行阶段各 Actor 仅依赖局部观测.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{mhd_mappo_arch.pdf}
  \vspace{4cm}
...
图~\ref{fig:hunt_train_curve} 比较了 MHD-MAPPO、vanilla MAPPO、MADDPG 以及基于
单头自注意力的 MAPPO 变体在 3v1 场景下的训练曲线. 可以看出, MHD-MAPPO 在约
$1.5\times 10^6$ 步后达到收敛, 而 vanilla MAPPO 需要 $2.5\times 10^6$ 步, MADDPG
甚至出现训练不稳定的现象. 同时, MHD-MAPPO 收敛时的平均奖励最高, 围捕成功率
达 92\%, 显著优于各基线方法.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{hunt_train_curve.pdf}
  \vspace{4cm}
...
图~\ref{fig:hunt_3v1_traj}-图~\ref{fig:hunt_4v2_traj} 分别展示了 3v1 与 4v2 场景
下的围捕过程快照与完整轨迹. 从轨迹图可以看出, 追击者先后经历了接近、成型、保持三
个阶段, 最终将逃逸者完全包围在 $\Omega_m(t)$ 内, 满足判据 4.1 的要求.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{hunt_3v1_traj.pdf}
  \vspace{4cm}
  \bicaption{3 对 1 协同围捕仿真轨迹}{Simulation trajectories of 3-vs-1 cooperative hunting}
  \label{fig:hunt_3v1_traj}
\end{figure}

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{hunt_4v2_traj.pdf}
  \vspace{4cm}
  \bicaption{4 对 2 协同围捕仿真轨迹}{Simulation trajectories of 4-vs-2 cooperative hunting}
  \label{fig:hunt_4v2_traj}
\end{figure}
...
\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.45]{usv_photo.pdf}
  \vspace{4cm}
  \bicaption{多无人艇实验平台实物照片}{Photograph of the multi-USV physical platform}
  \label{fig:usv_photo}
\end{figure}
...
广播自身状态, 实现必要的邻居信息交互. 系统整体架构如图~\ref{fig:system_arch} 所
示. 控制层分两级: 上层 RL 策略给出期望的纵向速度与艏向角速度, 下层通过 PID 控制
器将指令映射为左右电机推力.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.5]{system_arch.pdf}
  \vspace{4cm}
  \bicaption{多无人艇实验系统架构}{System architecture of the multi-USV experimental platform}
...
图~\ref{fig:hunt_2v1_real} 展示了 2 对 1 实物实验中记录的 GPS 轨迹与关键时刻快
照. 可以看出, 两艘追击者分别从左侧与右侧接近逃逸者, 在 $t\approx 18$ s 时开始进
入成型阶段, 分别占据逃逸者左右 ET 位置; 在 $t\approx 35$ s 时满足判据 4.1, 围
捕成功. 10 次重复实验的平均围捕时间为 $37.8$ s, 成功率为 $90\%$.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.45]{hunt_2v1_real.pdf}
  \vspace{4cm}
  \bicaption{2 对 1 协同围捕实物实验结果}{Real-world experimental results of 2-vs-1 cooperative hunting}
  \label{fig:hunt_2v1_real}
\end{figure}

\subsection{3 对 1 协同围捕实验结果}
\label{subsec:3v1_exp}
图~\ref{fig:hunt_3v1_real} 展示了 3 对 1 实物实验结果. 三艘追击者呈三角形分布将
逃逸者包围在中心区域, 从 $t\approx 22$ s 开始稳定维持包围编队直至捕获. 由于多艇
协同效果更好, 3 对 1 实验的平均围捕时间仅为 $29.5$ s, 成功率达 $95\%$, 优于 2 对
1 场景.

\begin{figure}[!htbp]
  \centering
  % \includegraphics[scale=0.45]{hunt_3v1_real.pdf}
  \vspace{4cm}
  \bicaption{3 对 1 协同围捕实物实验结果}{Real-world experimental results of 3-vs-1 cooperative hunting}
  \label{fig:hunt_3v1_real}
\end{figure}
