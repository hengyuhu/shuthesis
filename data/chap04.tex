...
第三章针对单艇对单艇追逃博弈提出了基于生物启发 PPO 的策略, 解决了单艇层面的
策略学习问题. 然而在实际海上场景中, 单艘追击者往往难以应对机动灵活的逃逸者,
更普遍的情形是多艘追击者协同对一艘或多艘逃逸者的\textbf{多无人艇协同围捕博弈}.
该任务在前者基础上更进一步, 不仅要求追击者逼近逃逸者, 还需要多艇之间在空间上
形成包围态势, 共同限制逃逸者的机动空间直至完成捕获.
第~\ref{subsec:hunt_review} 节已分析了现有协同围捕研究在围捕判据、阶段性奖励设
计以及工程验证等方面的不足. 针对这些不足, 本章围绕多无人艇协同围捕博弈策略的设
计与验证, 开展以下四方面工作:
(i) 在六自由度欠驱动运动学模型的基础上, 给出围捕成功的显式数学判据;
(ii) 提出能量波峰 (Energy Crest, EC) 与能量波谷 (Energy Trough, ET) 概念, 将威
胁势场转化为反映接近-成型-保持三阶段的密集奖励机制;
(iii) 在 MAPPO 算法的 Critic 网络中引入多头注意力机制, 形成 MHD-MAPPO 算法;
(iv) \textbf{搭建多无人艇物理实验平台, 在真实水面环境中完成 2 对 1 和 3 对 1 协
同围捕实物实验}, 从仿真到实物全面验证所提算法. 本章内容在作者已发表
...
\begin{itemize}
  \item \textbf{接近阶段}: 追击者与逃逸者相距较远, 目标是迅速缩短距离, 此阶段奖励侧重距离缩减分量;
  \item \textbf{成型阶段}: 追击者已接近逃逸者, 需快速占据各自对应的 ET 位置形成初步合围, 此阶段强化 ET 占位与编队维持奖励;
  \item \textbf{保持阶段}: 追击者已占据 ET 位置, 需维持包围态势并持续压缩 $R_e^m(t)$ 直至满足判据 4.1, 此阶段强化包围完整性奖励.
\end{itemize}
阶段之间的切换由平均追-逃距离、追击者占据 ET 的比例与编队紧密度等指标判断. 综合
上述三阶段围捕特点, 本章设计如下分阶段自适应奖励
\begin{equation}
...
\begin{itemize}
  \item \textbf{距离奖励} $r_{\mathrm{dist}}$: 鼓励追击者缩短与目标逃逸者的距离, 在阶段 1 中占主导地位;
  \item \textbf{ET 占位奖励} $r_{\mathrm{ET}}$: 当追击者进入 ET 区域时给予正奖励, 鼓励占据理想合围位置, 在阶段 2 与阶段 3 中权重较高;
  \item \textbf{编队维持奖励} $r_{\mathrm{form}}$: 奖励追击者形成均匀分布的包围编队, 避免扎堆;
  \item \textbf{避障/避碰惩罚} $r_{\mathrm{obs}}$: 对接近障碍物或其他追击者的行为给予连续惩罚;
  \item \textbf{终止奖励} $r_{\mathrm{term}}$: 判据 4.1 成立时给予较大正奖励, 超时或发生碰撞时给予较大负奖励.
\end{itemize}

\section{基于多头注意力的 MHD-MAPPO 算法}
\label{sec:mhd_mappo}

\subsection{MAPPO 框架与 Critic 中的多头注意力机制}
\label{subsec:mha_critic}
多智能体近端策略优化 (MAPPO) 算法 (参见 2.4.3 节) 为每个智能体配备独立的 Actor,
共享一个基于全局状态的 Critic. 在协同围捕场景中, 各 Actor 以局部观测 $o_i$ 为输
入输出动作 $a_i$, Critic 以全局联合状态 $\mathbf{s} = [o_1, o_2, \ldots, o_N]$
为输入输出状态值 $V_\phi(\mathbf{s})$, 协助 Actor 进行策略更新. 然而, 当智能体
数量较多且观测含有大量冗余信息时 (例如远处的追击者和当前决策相关性较弱), 传统
Critic 的全连接结构难以自适应地提取关键特征, 影响价值估计的准确性. 为此, 本章在
Critic 中引入多头注意力机制.
...
在 3v1 协同围捕场景下, 图~\ref{fig:hunt_compare} 展示了在相同初始条件下, 确定性算法、启发式算法与本文
...
\begin{figure}[!htbp]
  \centering
  \subcaptionbox{$t=20$ s (接近阶段)\label{fig:hunt_t20}}{\includegraphics[width=0.48\linewidth]{参考资料/TASE_终稿提交版/pic1.png}}
  \hfill
  \subcaptionbox{$t=50$ s (成型阶段)\label{fig:hunt_t50}}{\includegraphics[width=0.48\linewidth]{参考资料/TASE_终稿提交版/pic2.png}}

  \vspace{6pt}
  \subcaptionbox{$t=80$ s (保持阶段)\label{fig:hunt_t80}}{\includegraphics[width=0.48\linewidth]{参考资料/TASE_终稿提交版/pic3.png}}
...
为验证算法的泛化能力, 在训练集以外的障碍物分布、无人艇初始位置与逃逸者速度组合
下进行测试, 每种配置运行多次独立实验, 统计成功率、平均围捕时间与平均路径长度,
结果如表~\ref{tab:hunt_generalization} 所示. 实验表明, 所提算法在未见过的测试环
境下仍能保持较高的围捕成功率, 具备良好的泛化能力.

\begin{table}[htb]
  \centering
  \begin{minipage}[t]{0.9\linewidth}
    \caption[不同测试配置下的泛化性实验结果]{不同测试配置下的泛化性实验结果\protect\footnote{表中具体数值待按实际实验数据填入.}}
...
为验证多头注意力与阶段自适应奖励机制各自的贡献, 进一步开展消融实验. 分别构造以下变体:
...
\begin{table}[htb]
  \centering
  \begin{minipage}[t]{0.9\linewidth}
    \caption[协同围捕消融实验结果]{协同围捕消融实验结果\protect\footnote{表中具体数值待按实际实验数据填入.}}
...
\begin{itemize}
  \item \textbf{领域随机化 (Domain Randomization)}: 在训练阶段随机采样无人艇质量、推进力矩响应、传感器测量噪声及海流速度等参数, 使策略对参数变化具有鲁棒性;
  \item \textbf{动作延迟补偿}: 在仿真中对控制指令施加 $50\sim 200$ ms 的随机延迟, 模拟真实通信与控制链路的时滞;
  \item \textbf{观测噪声注入}: 对仿真观测添加与真实传感器相当的白噪声与偏差漂移;
  \item \textbf{微调训练}: 在物理平台上采集少量实验数据, 对策略网络进行小步长迭代微调, 进一步提升实物表现.
\end{itemize}

\subsection{实验场景与方案}
\label{subsec:hunt_exp_scene}
实验在上海大学无人艇试验水池与附近湖面开展. 实验水域四周由安全警戒浮标围成, 以
防无人艇冲出测试区域. 每组实验重复多次, 记录各无人艇的 GPS 轨迹、速度与艏向数
据, 并同步摄影存档.

\subsection{实物实验结果与鲁棒性分析}
\label{subsec:exp_results}

\textbf{(1) 运动状态分析.} 图~\ref{fig:hunt_exp_state} 展示了物理实验中各追击者的纵向速度与艏向角速度随时
间变化曲线. 可以观察到: 在接近阶段, 追击者以较高速度快速逼近逃逸者, 艏向频繁调
整以修正追击方向; 在成型阶段, 追击者速度略有降低, 艏向趋于稳定, 占据 ET 位置;
在保持阶段, 追击者速度与艏向均趋于平稳, 形成稳定的合围编队. 上述运动数据与仿真
结果一致, 体现了 Sim2Real 迁移后的策略可靠性.
...
\textbf{(2) 2 对 1 围捕.} 在 2 对 1 实物实验中, 两艘追击者分别从左侧与右侧接
近逃逸者, 随后相继进入成型阶段并分别占据逃逸者左右两侧的 ET 位置; 最终双方态势
满足判据 4.1, 围捕成功. 多次重复实验的统计结果表明所提算法能够稳定完成 2 对 1
协同围捕.

\textbf{(3) 3 对 1 围捕.} 在 3 对 1 实物实验中, 三艘追击者呈三角形分布将逃逸
者包围在中心区域, 并稳定维持包围编队直至完成捕获. 得益于更多的协同兵力, 3 对 1
场景相比 2 对 1 场景具有更快的围捕速度与更高的成功率.

\textbf{(4) 鲁棒性分析.} 表~\ref{tab:hunt_exp_summary} 总结了不同实验条件下的
成功率与围捕时间. 即便在轻微风浪等扰动条件下, 所提算法仍能保持较高的围捕成功率,
充分验证了 MHD-MAPPO 算法在实际水面环境中的工程可行性.

\begin{table}[htb]
  \centering
  \begin{minipage}[t]{0.95\linewidth}
    \caption[不同实验条件下物理实验结果汇总]{不同实验条件下物理实验结果汇总\protect\footnote{表中具体数值待按实际实验数据填入.}}
...
海洋环境中的博弈对抗奠定了算法与工程基础, 同时也为下一章引入第三方角色 (目标-进
攻者-防御者) 的非对称三方博弈研究提供了多智能体协同决策与注意力建模的方法支撑.
