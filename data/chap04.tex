...
在 2 对 1 实物实验中, 两艘追击者分别从左侧与右侧接近逃逸者, 随后相继进入成型阶
段并分别占据逃逸者左右两侧的 ET 位置; 最终双方态势满足判据 4.1, 围捕成功. 多次
重复实验的统计结果表明所提算法能够稳定完成 2 对 1 协同围捕.

\subsection{3 对 1 协同围捕实验结果}
\label{subsec:3v1_exp}
在 3 对 1 实物实验中, 三艘追击者呈三角形分布将逃逸者包围在中心区域, 并稳定维持
包围编队直至完成捕获. 得益于更多的协同兵力, 3 对 1 场景相比 2 对 1 场景具有更快
的围捕速度与更高的成功率.

\subsection{实物实验鲁棒性分析}
\label{subsec:hunt_exp_robust}
表~\ref{tab:hunt_exp_summary} 总结了不同实验条件下的成功率与围捕时间. 即便在轻
微风浪等扰动条件下, 所提算法仍能保持较高的围捕成功率, 充分验证了 MHD-MAPPO 算法
在实际水面环境中的工程可行性.
